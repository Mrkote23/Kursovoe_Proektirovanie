...
void decart(int** mas1, int** mas2, int men, int bol) {
	...
	int** result = (int**)malloc((men * bol) * sizeof(int*));
	for (int i = 0; i < (men * bol); i++) {
		result[i] = (int*)malloc((men * bol) * sizeof(int));
	}
	...
	if (bol == men) {
		...
		for (int i = 0; i < (bol); i++) {
			...
			for (int j = 0; j < (bol); j++) {
				...
				if (mas1[i][j] == 1) { ch1 = ch1 + 1; }
				if (mas2[i][j] == 1) { ch2 = ch2 + 1; }
			}
		}
	}
	...
	if (ch1 < ch2) {
		...
		int** mastemp = (int**)malloc((bol) * sizeof(int*));
		for (int i = 0; i < (bol); i++) {
			mastemp[i] = (int*)malloc((bol) * sizeof(int));
		}
		...
		for (int i = 0; i < (bol); i++) {
			...
			for (int j = 0; j < (bol); j++) {
				...
				mastemp[i][j] = mas1[i][j];
				mas1[i][j] = mas2[i][j];
				mas2[i][j] = mastemp[i][j];
			}
		}
	...
	for (int i = 0; i < (bol * men); i++) {

		for (int j = 0; j < (bol * men); j++) {
	...
	int mn = 0, i = 0, j = 0, z = 0, x = 0;

	for (int k = 0; k < men; k++) {

		for (z = 0, i = (i + mn); z < (bol); i++, z++) {

			for (x = 0, j = (0 + mn); x < (bol); j++, x++) {
	...
	for ( i = 0; i < (men); i++, verI = verI + bol, verJ = verJ + bol) {
